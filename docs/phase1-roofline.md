...
- Implement a single MAC PE in SystemC (`SC_MODULE`, `sc_fifo<float>`, weight-stationary)
- Wire N×N PEs into a systolic array; activations stream right, partial sums accumulate down
- Validate simulation output against `gemm_naive` within 1e-4
- Log cycle count, utilization %, stall cycles; parse with `analysis/cycle_stats.py`
