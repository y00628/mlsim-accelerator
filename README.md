...
AI is counted as FLOPs / bytes *actually loaded from memory* — different for each implementation.
Naive re-reads every element on every pass (4×M×K×N accesses); tiled loads each element once (M×K + K×N + M×N).

| M | K | N | Naive AI (FLOP/B) | Tiled AI (FLOP/B) | Naive (GFLOP/s) | Tiled-64 (GFLOP/s) | Speedup |
|--:|--:|--:|------------------:|------------------:|----------------:|-------------------:|--------:|
| 64 | 64 | 64 | 0.125 | 10.7 | 2.09 | 28.75 | **13.8×** |
| 128 | 128 | 128 | 0.125 | 21.3 | 3.32 | 19.78 | **6.0×** |
| 256 | 256 | 256 | 0.125 | 42.7 | 2.83 | 29.54 | **10.4×** |
| 512 | 512 | 512 | 0.125 | 85.3 | 2.79 | 22.94 | **8.2×** |
| 64 | 64 | 512 | 0.125 | 15.1 | 4.84 | 25.81 | **5.3×** |
| 512 | 128 | 512 | 0.125 | 42.7 | 3.51 | 26.81 | **7.6×** |

**Takeaways**
- **AI is implementation-defined** — naive's AI is a constant 0.125 FLOP/B (always memory-bound); tiled's AI grows with matrix size because cache reuse amortizes the fixed working set over more FLOPs.
- **Naive is deep in the memory-bound regime** — 0.125 FLOP/B sits far below the typical ridge point (~10–15 FLOP/B on M-series); the CPU spends most cycles waiting on memory, not computing.
- **Tiling recovers cache reuse** — tile=64 achieves 8–14× speedup by keeping each tile in L1/L2 for the full inner loop, pushing AI above the ridge point.
